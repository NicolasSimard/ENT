...
GG_qexp(k,pr) = -bernfrac(k)/2/k+Ser(concat([0],vector(pr-1,n,sigma(n,k-1))),'q);

E(k,s) = -2*k/bernfrac(k)*GG(k,s);

E_qexp(k,pr) = -2*k/bernfrac(k)*GG_qexp(k,pr);
...
*/
theta_qexp(pr) = Ser(vector(pr,n,2*issquare(n-1)),'q)-1;
...
*/
delta_qexp(pr) = 'q*sqr(sqr(sqr(eta3_qexp(pr-1))));

/* Takes 213 ms to compute 1000 coefficients.*/
j_qexp(pr) = E_qexp(4,pr+2)^3/delta_qexp(pr+2);
